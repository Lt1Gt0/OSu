...
{
  // get the total map entries by dividing the size of the map by the descriptor
  // size
  ...
  // Get the kernel size
  uint64 kernelSize = (uint64)&_KERNEL_END_ - (uint64)&_KERNEL_START_;
  uint64 kernelPages = (uint64)kernelSize / 0x1000 + 1;

  // Lock the pages that correspond to kernel to prevent corrupting the kernel's
  // code
  GlobalAllocator.LockPages(&_KERNEL_START_, kernelPages);
  ...
  for (uint64 t = 0;
       t < GetMemorySize(bootInfo->mMap, mMapEntries, bootInfo->mMapDescSize);
       t += 0x1000) {
  ...
  for (uint64 t = fbBase; t < fbBase + fbSize; t += 0x1000) {
    PageTableManager::MapMemory(
        (void *)t, (void *)t); // Map the frame buffer pages to virtual memory
  }
...
IDTR idtr;
void SetIDTGate(void *handler, uint8 entryOffset, uint8 type_attr,
                uint8 selector) {
  IDTDescEntry *interrupt =
      (IDTDescEntry *)(idtr.Offset + entryOffset * sizeof(IDTDescEntry));
...
void PrepareInterrupts(BootInfo *bootInfo) {
  ...
  SetIDTGate((void *)BoundRangeExceeded_Handler, 0x05, IDT_TA_InterruptGate,
             0x08);
  SetIDTGate((void *)InvalidOpcode_Hander, 0x06, IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)DeviceNotAvaliable_Handler, 0x07, IDT_TA_InterruptGate,
             0x08);
  SetIDTGate((void *)DoubleFault_Handler, 0x08, IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)CoprocessorSegmentOverrun_Handler, 0x09,
             IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)InvalidTSS_Handler, 0x0A, IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)SegmentNotPresent_Handler, 0x0B, IDT_TA_InterruptGate,
             0x08);
  SetIDTGate((void *)StackSegmentFault_Handler, 0x0C, IDT_TA_InterruptGate,
             0x08);
  ...
  SetIDTGate((void *)x87FloatingPointException_Handler, 0x10,
             IDT_TA_InterruptGate, 0x08);
  ...
  SetIDTGate((void *)SIMDFloatingPointException_Handler, 0x13,
             IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)VirtualizationException_Handler, 0x14,
             IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)ControlProtectionException_Handler, 0x15,
             IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)HypervisorInjectionException_Handler, 0x1C,
             IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)VMMCommunicationException_Handler, 0x1D,
             IDT_TA_InterruptGate, 0x08);
  SetIDTGate((void *)SecurityException_Handler, 0x1E, IDT_TA_InterruptGate,
             0x08);
...
void PrepareACPI(BootInfo *bootInfo) {
  ...
  ACPI::MADTHeader *madt =
      (ACPI::MADTHeader *)ACPI::FindTable(xsdt, (char *)"APIC");
